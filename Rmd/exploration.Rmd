...
reshpDT <- data.table::dcast(wDT[,.(Year, Species, ISO_Alpha, Exp_Alpha, value)], Year+Species+Exp_Alpha ~ ISO_Alpha, value.var = "value")

reshpDT[,rowID := paste0(Exp_Alpha, Year, Species, collapse = "_")]
...
#Plotting curved trade paths in leaflet (geosphere)

inter <- gcIntermediate(c(5,52), c(-120,37),
               n=30, 
               addStartEnd=TRUE,
               sp=FALSE)
inter2 <- gcIntermediate(c(100,52), c(120,37),
               n=50, 
               addStartEnd=TRUE,
               sp=FALSE)

#testDT  <- as.matrix(rbindlist(list(as.data.table(inter), as.data.table(list('lon' = NA, 'lat' = NA)), as.data.table(inter2)), use.names = T))

#Can't merge bc it's not a unique list of countries...
#worldData <- merge(world, wDT, by.x = "iso_a3", by.y = "ISO_Alpha")
...
mapDT[,
  spLine := 
    paste0('SpatialLines(list(Lines(list(Line(rbind(c(', 
      ...
      i.Long, ',', 
      i.Lat, ')))), ID = ', 
      .I, ')),proj4string = CRS("+init=epsg:4326"))'
    )  
]
...
#Actually, we want our lst to change on click, with polytile erasing lines (cleaner look); for now, use example of JPN
thisISO <- 'JPN'
...
#
#lst <- lapply(1:length(spLines), function(i){
#  fullStr <- paste0(spLines[i], )
#  y <- eval(parse(text = fullStr))
#  return(y)
#})


#in this instance, we look at species a to JPN for 2011... this doesn't really work tho
#world@data$value <- wDT[worldDT][(Year == max(wDT[, Year])) | is.na(Year)][ISO_Alpha == 'JPN' | is.na(Export)][(Species == 'a') | is.na(Species), value]
#testDT <- as.data.frame(wDT[worldDT][!is.na(Species)])

#use merge method as in https://stat.ethz.ch/pipermail/r-sig-geo/2008-January/003052.html
#combined <- merge(x=world@data, y=wDT, by.x='iso_a3', by.y='ISO_Alpha')
#world@data <- combined
...
addPolygons(data=subset(world, iso_a3 %in% wDT[, ISO_Alpha]), weight=subset(world, iso_a3 %in% wDT[, ISO_Alpha])@data['value'])

  #addPolygons(data=subset(world, ISO_Alpha %in% wDT[, ISO_Alpha]), weight=subset(world, iso_a3 %in% wDT[, ISO_Alpha])@data['mapcolor7'])

#based on http://stackoverflow.com/questions/31311063/how-to-add-polylines-from-one-location-to-others-separately-using-leaflet-in-shi 

root <- matrix(c(-197.0563, -8.4171), ncol = 2)
colnames(root) <- c('Long', 'Lat')
#locations <- data.frame(Long = 15:20, Lat = c(10, 40, 120, -15, 80))

locations <- coordinates(world)
...
#addTiles() %>% 
#addPolylines(
#  lng = c(testDT[1, lon],testDT[32, lon]), 
#  lat = c(testDT[1, lat],testDT[32, lat]), 
#  weight = 1)
...
#...or we could trick it using greatCircle
#mapDT[ISO_Alpha == thisISO, 
trickPoints <- SpatialPoints(mapDT[,data.frame(c(Lat, Long), c(i.Lat, i.Long))])
